...
class Window(QDialog):
    ...
    # validate
    def Validate(self):
        self.target = int(self.targetEdit.toPlainText())    # read target (input) number of detected objects
        self.validate = 1
    ...
    def ImageUpdateSlot(self, Coords, Frame, Image, DetectionsNumber):
        ...
        # debug if number of detections != target
            ...
            self.logEdit.append(f"Missmatch : Logged successfully: {name}")
            ...
            if isTrue:
                ...
                for i in range(len(scores)):
                    if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                        ...
                        label = '%s: %d%%' % (object_name, int(scores[i]*100)) # Example: 'card: 72%'
